"""
    ...
    copyright: (c) 2022 by Stephen Rothery
...
def c32encode(input_hex, min_length=None):
    ...
    res = ['0'] * int(num_leading_byte_zeros) + res
...
def c32checkEncode(version, data):
    ...
    checksum_hex = c32checksum('{}{}'.format(version_hex, data.decode()))
    c32str = c32encode('{}{}'.format(data.decode(), checksum_hex))
...
def b58ToC32(b58check, version=-1):
    ...
    'SN2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKP6D2ZK9'
    """
    address_data = base58.b58decode_check(b58check)
    addr_version_byte = address_data[:1]
    addr_version = ord(addr_version_byte)
    addr_hash160 = binascii.hexlify(address_data[1:])
...
def c32ToB58(c32string, version=-1):
    ...
    return base58.b58encode_check(bitcoin_version.to_bytes(1, byteorder='big') + binascii.unhexlify(addr_hash160))
