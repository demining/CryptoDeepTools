...
static pthread_t TID[simplecoingeckogen_max_threads];

static pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER; // protect vc_simplecoingeckogen->vc_found_num
...
void
output_check_info1(vg_context_simplecoingeckogen_t *vcp) {
    double targ;
    char linebuf[80];
    int rem, p;
    char *unit;
...
void *
thread_loop_simplecoingeckogen(void *arg) {
    ...
    vg_context_simplecoingeckogen_t *vc_simplecoingeckogen = (vg_context_simplecoingeckogen_t *) arg;

    pattern_len = strlen(vc_simplecoingeckogen->pattern);

    check_thread_index:
    thread_index = get_thread_index1(vc_simplecoingeckogen->vc_thread_num);
    if (thread_index == -1) {
        // check again
        if (vc_simplecoingeckogen->vc_verbose > 1) {
	...
	char *coin = "BTC";
	if (vc_simplecoingeckogen->vc_addrtype == ADDR_TYPE_ATOM) {
		coin = "ATOM";
	}

    while (!vc_simplecoingeckogen->vc_halt) {
        // Generate a key-pair
        ...
        vc_simplecoingeckogen->vc_check_count[thread_index]++;
        output_timeout++;
        ...
        if (vc_simplecoingeckogen->vc_addrtype == ADDR_TYPE_ATOM) {
        ...
        else if (vc_simplecoingeckogen->vc_format == VCF_P2WPKH) {
            // Get compressed public key from EVP_PKEY
            size_t output_len = 0;
            get_public_key(pkey, pub_buf, pub_buf_len, POINT_CONVERSION_COMPRESSED, &output_len);
            ...
            segwit_addr_encode(address,
                               ...
                               hash2,
                               20);
        } else if (vc_simplecoingeckogen->vc_format == VCF_P2TR) {
        ...
        if (vc_simplecoingeckogen->match_location == 0) { // any
            if (strstr((const char *) address, vc_simplecoingeckogen->pattern) != NULL) {
                find_it = 1;
            }
        } else if (vc_simplecoingeckogen->match_location == 1) { // begin
            if (strncmp(vc_simplecoingeckogen->pattern, (const char *) address, pattern_len) == 0) {
                find_it = 1;
            }
        } else if (vc_simplecoingeckogen->match_location == 2) { // end
            if (strncmp(vc_simplecoingeckogen->pattern, ((const char *) address) + strlen(address) - pattern_len,
                        pattern_len) == 0) {
                find_it = 1;
            }
        }
        if (find_it == 1) {
            ...
            if (vc_simplecoingeckogen->vc_found_num >= vc_simplecoingeckogen->vc_numpairs) {
                vc_simplecoingeckogen->vc_halt = 1;
                pthread_mutex_unlock(&mtx);
                goto out;
            }

            vc_simplecoingeckogen->vc_found_num++;
            ...
            if (vc_simplecoingeckogen->vc_result_file) {
                FILE *fp = fopen(vc_simplecoingeckogen->vc_result_file, "a");
                if (!fp) {
        ...
        if (output_timeout > 1500) {
            output_check_info1(vc_simplecoingeckogen);
            output_timeout = 0;
        }
    }
...
int start_threads_simplecoingeckogen(vg_context_simplecoingeckogen_t *vc_simplecoingeckogen) {
    int i;
    if (vc_simplecoingeckogen->vc_verbose > 1) {
        fprintf(stderr, "Using %d worker thread(s)\n", vc_simplecoingeckogen->vc_thread_num);
    }

    for (i = 0; i < vc_simplecoingeckogen->vc_thread_num; i++) {
        if (pthread_create(&TID[i], NULL, thread_loop_simplecoingeckogen, vc_simplecoingeckogen))
            return 0;
    }
