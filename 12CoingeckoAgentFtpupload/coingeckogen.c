/*
 * coingeckogen, vanity bitcoin address generator
 * Copyright (C) 2011 <samr7@cs.washington.edu>
 *
 * coingeckogen is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * any later version.
 *
 * coingeckogen is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with coingeckogen.  If not, see <http://www.gnu.org/licenses/>.
 */
...
const char *version = coingeckogen_VERSION;
...
{
	fprintf(stderr,
"coingeckogen %s (" OPENSSL_VERSION_TEXT ")\n"
...
/*BEGIN ALTCOIN GENERATOR*/
		...
		case 'C':
			...
			if (strcmp(optarg, "LIST")== 0) {
				fprintf(stderr,
					"Usage example \"./coingeckogen -C ETH 0x1234\"\n"
...
/*END ALTCOIN GENERATOR*/
	...
	if (addrtype == ADDR_TYPE_XLM) {
        ...
        if (vc_ed25519->match_location == 1 && vc_ed25519->pattern[0] != 'G') {
            ...
            fprintf(stderr, "Hint: Run coingeckogen with \"-C LIST\" for a list of valid prefixes.  Also note that many coins only allow certain characters as the second character in the prefix.\n");
            return 1;
        }
	...
	if (addrtype == ADDR_TYPE_ATOM) {
		...
		vg_context_simplecoingeckogen_t *vc_simplecoingeckogen = NULL;
		vc_simplecoingeckogen = (vg_context_simplecoingeckogen_t *) malloc(sizeof(*vc_simplecoingeckogen));
		vc_simplecoingeckogen->vc_format = format;
		vc_simplecoingeckogen->vc_verbose = verbose;
		vc_simplecoingeckogen->vc_addrtype = addrtype;
		vc_simplecoingeckogen->vc_privtype = privtype;
		vc_simplecoingeckogen->vc_result_file = result_file;
		vc_simplecoingeckogen->vc_numpairs = numpairs;
		if (vc_simplecoingeckogen->vc_numpairs == 0) {
			vc_simplecoingeckogen->vc_numpairs = 1;
		}
		vc_simplecoingeckogen->pattern = *patterns;
		vc_simplecoingeckogen->match_location = 1; // By default, match begin location

		size_t pattern_len = strlen(vc_simplecoingeckogen->pattern);

		if (regex) {
			fprintf(stderr, "WARNING: only ^ and $ is supported in regular expressions currently\n");
			if (vc_simplecoingeckogen->pattern[0] == '^') {
				vc_simplecoingeckogen->match_location = 1; // match begin location
				// skip first char '^'
				vc_simplecoingeckogen->pattern = vc_simplecoingeckogen->pattern + 1;
			} else if (vc_simplecoingeckogen->pattern[pattern_len-1] == '$') {
				vc_simplecoingeckogen->match_location = 2; // match end location
				// remove last char '$'
				vc_simplecoingeckogen->pattern[pattern_len-1] = '\0';
			} else {
				vc_simplecoingeckogen->match_location = 0; // match any location
			}
		}
		...
		if (nthreads <= 0) {
			...
			if (nthreads > simplecoingeckogen_max_threads) {
				fprintf(stderr, "WARNING: too many threads\n");
				nthreads = simplecoingeckogen_max_threads;
			}
		}
		vc_simplecoingeckogen->vc_thread_num = nthreads;
		vc_simplecoingeckogen->vc_start_time = (unsigned long)time(NULL);

		if (!start_threads_simplecoingeckogen(vc_simplecoingeckogen))
			return 1;
		...
		if (vc_simplecoingeckogen->match_location == 1) {
			if (vc_simplecoingeckogen->vc_format == VCF_P2WPKH && strncmp(vc_simplecoingeckogen->pattern, "bc1q", strlen("bc1q")) != 0) {
				fprintf(stderr, "Prefix '%s' not possible, address must starts with bc1q for p2wpkh\n", vc_simplecoingeckogen->pattern);
				return 1;
			} else if (vc_simplecoingeckogen->vc_format == VCF_P2TR && strncmp(vc_simplecoingeckogen->pattern, "bc1p", strlen("bc1p")) != 0) {
